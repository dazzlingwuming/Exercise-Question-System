...
MATH_MARKDOWN_INSTRUCTION = (
    "\n【数学公式输出硬性格式】如果回答中包含数学公式，必须遵守以下规则："
    "行内公式完整包在成对的 `$...$` 中，例如 `信息量为 $I(x)=-\\log_2 p(x)$。`；"
    "块级公式必须独占一行，并使用成对的 `$$`，例如：`$$\\mathbb{E}[X]=\\sum_x xP(x)$$`。"
    "不要使用 `\\(...\\)`、`\\[...\\]`、单独裸露的 `\\log`/`\\sum`/`^`/`_`，也不要把 `$$` 放在中文句子中间。"
    "公式定界符必须成对闭合；中文解释、句号和 Markdown 粗体必须放在公式定界符外。"
    "输出前自检：每个 `$` 或 `$$` 都已闭合，块公式前后有换行，所有 LaTeX 命令都位于公式定界符内部。"
    "【Markdown 输出硬性格式】标题必须写成 `## 标题`（井号后必须有空格），列表每项必须独占一行并以 `- ` 开头，"
    "需要强调的选项必须写成 `- **A 正确**：说明`，不要输出裸的 `##标题`、`**` 残片或把标题和列表拼在同一行。"
)
