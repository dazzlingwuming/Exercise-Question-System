...
讲题助手和评分追问都支持流式输出。AI 配置通过 DeepSeek OpenAI-compatible API 调用，API Key 只保存在浏览器 `sessionStorage` 或通过环境变量提供，不入库。
...
可以在网页右侧 AI 面板里填写 DeepSeek 配置。Key 仅保存在当前浏览器 `sessionStorage`。
...
- `DEEPSEEK_MODEL` 用于 AI 讲题助手。
- `DEEPSEEK_GRADING_MODEL` 用于主观题评分和评分追问。
...
- 个人面试题库管理
- 技术理论知识复盘
- Agent / RAG / MCP / Tool Calling 等方向的专项练习
- 主观题表达打磨
- 错题反复训练
